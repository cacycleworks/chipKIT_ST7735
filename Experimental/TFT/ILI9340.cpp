...
void ILI9340::initializeDevice() {
    ...
    _comm->writeCommand(0xF7);
    _comm->writeData8(0x20);

    _comm->writeCommand(0xEA);
    _comm->writeData8(0x00);
    _comm->writeData8(0x00);

    _comm->writeCommand(ILI9340_PWCTR1); //Power control
    _comm->writeData8(0x23); //VRH[5:0]

    _comm->writeCommand(ILI9340_PWCTR2); //Power control
    _comm->writeData8(0x10); //SAP[2:0];BT[3:0]

    _comm->writeCommand(ILI9340_VMCTR1); //VCM control
    _comm->writeData8(0x3e); //¶Ô±È¶Èµ÷½Ú
    _comm->writeData8(0x28);

    _comm->writeCommand(ILI9340_VMCTR2); //VCM control2
    _comm->writeData8(0x86); //--

    _comm->writeCommand(ILI9340_MADCTL); // Memory Access Control
    _comm->writeData8(ILI9340_MADCTL_MX | ILI9340_MADCTL_BGR);

    _comm->writeCommand(ILI9340_PIXFMT);
    _comm->writeData8(0x55);

    _comm->writeCommand(ILI9340_FRMCTR1);
    _comm->writeData8(0x00);
    _comm->writeData8(0x18);

    _comm->writeCommand(ILI9340_DFUNCTR); // Display Function Control
    _comm->writeData8(0x08);
    _comm->writeData8(0x82);
    _comm->writeData8(0x27);

    _comm->writeCommand(0xF2); // 3Gamma Function Disable
    _comm->writeData8(0x00);

    _comm->writeCommand(ILI9340_GAMMASET); //Gamma curve selected
    _comm->writeData8(0x01);

    _comm->writeCommand(ILI9340_GMCTRP1); //Set Gamma
    ...
    _comm->writeCommand(ILI9340_GMCTRN1); //Set Gamma
    _comm->writeData8(0x00);
    _comm->writeData8(0x0E);
    ...
    _comm->writeCommand(ILI9340_SLPOUT); //Exit Sleep
    delay(120);
    _comm->writeCommand(ILI9340_DISPON); //Display on 
}

void ILI9340::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
	_comm->writeCommand(ILI9340_CASET); // Column addr set
...
//	_comm->writeData8(x1 & 0xFF);     // XEND
	...
	_comm->writeCommand(ILI9340_PASET); // Row addr set
...
//	_comm->writeData8(y1 & 0xFF);     // YEND
	...
	_comm->writeCommand(ILI9340_RAMWR); // write to RAM
}
...
void ILI9340::setRotation(uint8_t m) {
	_comm->writeCommand(ILI9340_MADCTL);
...
void ILI9340::invertDisplay(boolean i) {
	_comm->writeCommand(i ? ILI9340_INVON : ILI9340_INVOFF);
}
